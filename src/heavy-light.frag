// Use mouse to control gravity strength
// I didn't really clean it up
// see it here: http://editor.thebookofshaders.com/
...
#define GRAVITY -0.05 * ((u_mouse.y /u_resolution.y) - 0.5)
...
vec3 march(vec3 camera, vec3 ray) {
    ...
    for (float steps = 0.0; steps < float(STEPS); steps++) {
    	float distance = scene(camera);
        minDistance = min(minDistance, distance);
        depth += distance;
        camera = camera + distance * ray;


        vec3 horiz_plane = normalize(vec3(ray.x, 0.0, ray.z));
        float angle = acos(dot(ray, horiz_plane));
        float horiz_dist = cos(angle) * distance;
        // float vert_dist  = sin(angle) * distance * sign(ray.y);
        // vert_dist = horiz_dist * vert_dist + 0.5 * GRAVITY * horiz_dist * horiz_dist;
        // ray = vec3(ray.x * distance, vert_dist, ray.z * distance);

        ray.y -= GRAVITY * horiz_dist;
        ray = normalize(ray);
...
void main() {
    ...
    float i_time = u_time * TIMESCALE + 42321.0;

    // make a ray normal based off the coordinate
    // make a camera point
    vec3 ray = makeRay(FOV, u_resolution.x/u_resolution.y, st);
    vec3 camera = vec3(sin(i_time * 0.67) * ORBIT, 0.0, cos(i_time * 0.41) * ORBIT);
