...
// but I've tweaked it quite a lot to make it better
float mandelbulb(vec3 r ) {
	float POWER = sin(u_time * TIMESCALE) * 8.0;
    vec3 zn = vec3(r.xyz);
	float rad = 0.0;
	float hit = 0.0;
	float d = 1.0;
...
void main() {
	...
	float fov = -cos(i_time) * 50.0 + 70.0;    
    ...
    mat3 view = look(camera, vec3(0.0), vec3(1.0, 1.0, 1.0));
    vec3 dir = view * ray;
    ...
    vec3 color = vec3(st.x, st.y, abs(sin(i_time)));
	...
	color = vec3(color * pow(occolusion, 2.0));    
